...
class SAIPKDViTWrapper(nn.Module):
    ...
    def forward(self, base_imgs, meta):
        ...
        # b_aligned_feats, _ = self.adapter(b_feats[1])
...
class SAIPv1KDViTWrapper(nn.Module):
   
    def __init__(self, **args):
        super(SAIPv1KDViTWrapper, self).__init__()
        self.backbone = SAIPViT(**args)
    ...
    def forward(self, base_imgs, meta):
        ...
        aligned_cls_feats.append(msc_feats[:, 0, :])
...
def saipv1_kd_vit_tiny_patch16_adapt_vit_b(**kwargs): 
    model = SAIPKDViTWrapper(patch_size=(16, 16), embed_dim=192, depth=12, 
        num_heads=6, num_heads_in_last_block=12,
...
def saipv1_kd_vit_tiny_patch16_adapt_vit_l(**kwargs): 
    # print('saipv1_kd')
    model = SAIPKDViTWrapper(patch_size=(16, 16), embed_dim=192, depth=12, 
