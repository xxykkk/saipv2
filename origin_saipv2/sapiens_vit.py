...
class AttentionOutputMultiheadAttention(MultiheadAttention):
    def forward(self, x):
        B, N, _ = x.shape
        ...
        scale = (self.embed_dims // self.num_heads) ** -0.5
        qk = (q @ k.transpose(-2, -1)) * scale
        qk = qk.softmax(dim=-1)
        vv = ((v @ v.transpose(-2, -1)) * scale).softmax(dim=-1)
...
class TransformerEncoderLayer(BaseModule):
    ...
    def __init__(self,
        ...
        self.attn = AttentionOutputMultiheadAttention(
...
class VisionTransformer(BaseBackbone):
    ...
    def _prepare_pos_embed(self, state_dict, prefix, *args, **kwargs):
        ...
        ckpt_pos_embed_shape = state_dict[name].shape # (1024/16)*(768/16)=64*48=3072 torch.Size([1, 3072, 1536])
        ...
        if self.pos_embed.shape != ckpt_pos_embed_shape:
            ...
            ckpt_pos_embed_shape = to_2tuple(
                int(np.sqrt(ckpt_pos_embed_shape[1] - self.num_extra_tokens))) # 默认ckpt_pos_embed_shape的输入是方形的
