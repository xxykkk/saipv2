...
from util.ckpt import checkpoint_wrapper
...
class Block(nn.Module):
    ...
    def forward(self, x, H, W, mask=None):
        y, attn = self.attn(self.norm1(x), H, W)
...
class ViT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=80, embed_dim=768, depth=12,
                 ...
                 round_padding=False, compat=False, **args):
        super().__init__()
    ...
    def forward(self, input_var):
        ...
        if torch.min(x) >= 0 and torch.max(x) <= 255:
...
def vit_base_patch16(pretrained=False, rel_path="pretrain_weights/mae_pretrain_vit_base.pth", load_pos_embed=True, **kwargs):
    default = dict(
    ...
    if pretrained:
        ...
        checkpoint_model = torch.load(os.path.join(script_dir, rel_path), map_location='cpu')
        ...
        # param_dict = {k.replace("backbone_blocks", "blocks"): v for k, v in param_dict.items() if "backbone_blocks" in k}
        # new_param_dict = {k.replace("module.", ""): v for k, v in new_param_dict.items()}
        msg = model.load_state_dict(new_param_dict, strict=False)
        print('Load from {}: {}'.format(rel_path, msg))
        
        # load while interpolates position embedding
        # load_checkpoint(model, param_dict, load_pos_embed, strict=False, logger=dummy_logger)
...
def interpolate_pos_embed(pos_embed_checkpoint, patch_shape, num_extra_tokens):
    ...
    print(f"[rank {misc.get_rank()}] Position interpolate from {orig_size} to {patch_shape}")
