...
def main(args):
    ...
    student = models_pretrain.__dict__[args.model]()
    ema_teacher = models_pretrain.__dict__[args.model]()
    teacher.to(device)
    student.to(device)
...
def train_one_epoch(student, teacher, ema_teacher, ema_teacher_without_ddp, csm_kd_loss, momentum_schedule,
    ...
    for data_iter_step, (samples, _) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        with torch.cuda.amp.autocast():
            if target_size_idx>0:
                pred_t = teacher(samples[0], meta, extract_regions=False)
                pred_s = student(samples[0], meta, extract_regions=False)
                nimages = 1
                ncrops = 1
            else:
                pred_t = teacher(samples[0], meta)
                pred_s = student(samples[0], meta)
        m_loss = csm_kd_loss(pred_s['aligned_cls_feats'], pred_s['aligned_patch_feats'], 
                                pred_s['qkv_atten'], pred_t['feats_from_teacher'].detach(), 
                                pred_t['feats_from_teacher_patch'].detach(), pred_t['qkv_atten'],
                                nimages, ncrops)
        loss = m_loss['align_rep_loss'] + m_loss['align_att_loss']
