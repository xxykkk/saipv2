...
python -m torch.distributed.launch --nproc_per_node 8 --master_port=29501  main_align_sapiens_pretrain_ema_offline.py \
  --batch_size=128 --accum_iter=2 \
  --model=saip_kd_vit_tiny_patch16_sapiens_0_3b_ema \
  --data_path=data/HD1M \
  --feature_path=/mnt/nfs/HAG/wangxuanhan/datasets/preprocessed_HD1M/sapiens_0.3b_coco_best_coco_AP_796 \
  --norm_pix_loss \
  --mask_ratio=0.75 \
  --epochs=100 \
  --blr=2.5e-4 \
  ...
  --output_dir=work_dirs/hd1m_repa_align_expert_sapiens_100e_vit_tiny_train_ema_offline \
  --log_dir=work_dirs/hd1m_repa_align_expert_sapiens_100e_vit_tiny_train_ema_offline \
