...
def get_args_parser():
    ...
    parser.add_argument('--token_type', default='cls', type=str, help="Type of token.")
    parser.add_argument('--loss_type', default='l1', type=str, help="Type of loss function.")
...
def main(args):
    ...
    dataset_train = CustomDataset([os.path.join(args.data_path, 'train')], 
    ...
    print("#"*50, "student backbone param", "#"*50)
    count_parameters(student.module.backbone)
    print("#"*50, "student adapter param", "#"*50)
    count_parameters(student.module.adapter)
...
def train_one_epoch(student, teacher, ema_teacher, ema_teacher_without_ddp, csm_kd_loss, momentum_schedule,
    ...
    for data_iter_step, (samples, _) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        with torch.cuda.amp.autocast():
            ...
            pred_s_single = student(samples[0], meta, extract_regions=False, with_cls_token=True)
        # # single
        # pred_t_multi = teacher(samples[1], meta, extract_regions=False)
        # pred_s_multi = student(samples[1], meta, extract_regions=False)
        m_loss = csm_kd_loss(s_feats_cls = pred_s_single['aligned_cls_feats'], 
                             t_feats_cls = pred_t_single['feats_from_teacher'].detach(),
                             s_feats_patch = pred_s_single['aligned_patch_feats'], 
                             t_feats_patch = pred_t_single['feats_from_teacher_patch'].detach(),
                            nimages = nimages, ncrops = ncrops, 
                            token_type=args.token_type, loss_type=args.loss_type)
        ...
        for key, value in m_loss.items():
            m_loss_reduce[key] = misc.all_reduce_mean(m_loss[key].clone().detach())
...
class DataAugmentation(object):
    ...
    def __init__(self, size, crop_size, global_crops_scale, local_crops_scale, local_crops_number):
        ...
        # )
        self.global_transform = transforms.Compose([
            ResizeAndPadToAspectRatio(target_aspect_ratio=size[1]/size[0], target_size=size),
            transforms.ToTensor(),
            transforms.Normalize((0.485, 0.456, 0.406), (0.229, 0.224, 0.225))
        ])
    ...
    def __init__(self, image_folders, transform=None):
        ...
        for image_folder in self.image_folders:
            if not os.path.exists(image_folder):
                raise ValueError(f"{image_folder} not exites!")
...
class TheiaLossV2(nn.Module):
    ...
    def forward(self, s_feats_cls, t_feats_cls, s_feats_patch, t_feats_patch, nimages, ncrops, token_type, loss_type):
        ...
        if token_type == 'cls':
            l1_loss += F.huber_loss(s_feats_cls, t_feats_cls)
            
            s_feats_cls_norm = F.normalize(s_feats_cls, dim=-1)
            t_feats_cls_norm = F.normalize(t_feats_cls, dim=-1)
            cos_loss += torch.mean((1.-(s_feats_cls_norm * t_feats_cls_norm).sum(dim=-1)))
