...
def train_one_epoch(student, teacher, ema_teacher, ema_teacher_without_ddp, csm_kd_loss, momentum_schedule,
    ...
    for data_iter_step, (samples, _) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        with torch.cuda.amp.autocast():
            ...
            m_loss = csm_kd_loss(pred_s['align_feats'], pred_s['qkv_atten'], pred_t['feats_from_teacher'].detach(), pred_t['qkv_atten'])
            loss = m_loss['kd_feat_loss'] + m_loss['kd_att_loss']
        ...
        metric_logger.update(loss=loss_value, kd_feat_loss=m_loss['kd_feat_loss'].item(), kd_att_loss=100*m_loss['kd_att_loss'].item())
        ...
        for iq, q in enumerate(t_feats):
            for v in range(len(s_feats)):
                if v < 2 and v == iq:
                    ...
                    loss = nn.SmoothL1Loss(reduction="none")(s_feats[v], q).mean(-1)
                    rep_sim_loss += loss.mean()
                    n_rep_loss_terms += 1
        rep_sim_loss /= n_rep_loss_terms
        att_sim_loss /= n_att_loss_terms
        
        return {'kd_feat_loss':rep_sim_loss,  'kd_att_loss':0.1*att_sim_loss}
