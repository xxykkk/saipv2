...
class ExpertDinov2(nn.Module):
    def __init__(self, pretrained, model='dinov2_vitl14', **kwargs):
        super().__init__(**kwargs)
        # self.model = torch.hub.load(pretrained, model)
        self.model = torch.hub.load('/home/wangxuanhan/.cache/torch/hub/facebookresearch_dinov2_main', model, source='local')
...
def expert_sapiens_0_3b(patch_size=16, pretrained='', **kwargs):
    model = ExpertSapiens(
        arch='sapiens_0.3b', img_size=(1024, 1024), patch_size=patch_size, qkv_bias=True, 
        final_norm=True, drop_path_rate=0.0, with_cls_token=True, out_type='featmap', 
        patch_cfg=dict(padding=2), pretrained=pretrained, 
        **kwargs)
    return model
